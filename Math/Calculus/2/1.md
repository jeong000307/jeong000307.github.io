...
>
> $$\lim_{x \to a} f(x) = L \quad \Longleftrightarrow \quad \lim_{x \to a^-} f(x) = L = \lim_{x \to a^+} f(x)$$
>
> 이다.
...
[1] $$\lim_{x \to a} f(x) = L$$, $$\lim_{x \to a} g(x) = M$$이라고 하자. 그러면 임의의 양수 $\epsilon$에 대해

$$ 0 < |x - a| < \delta_1 \Rightarrow |f(x) - L| < \epsilon$$

$$ 0 < |x - a| < \delta_2 \Rightarrow |g(x) - M| < \epsilon$$

을 만족하는 $\delta_1$, $\delta_2$가 존재해야 할 것이다. $\delta_0 = \min \\{ \delta_1, \delta_2 \\}$라 하자. 그러면 원래 우리가 만족시켜야 하는 식인
...
자, 아까의 $\delta_0$을 $\delta$에 대입하고 삼각부등식을 이용해준다면
...
[2] 뺄셈의 경우에는 덧셈의 경우에서 $g(x)$만 $-g(x)$으로 바꾸면 되므로 쉽게 구할 수 있다.
