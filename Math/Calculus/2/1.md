...
'갑'과 '을'이 있다고 가정하자. '갑'과 '을'은 게임을 하려고 한다. '갑'의 차례에는 함숫값으로부터의 오차($\epsilon$)를 말해주고, '을'의 차례에는 
...
따라서 식을 잘 정리하면 $\lim_{x \to a} f(x)g(x) = LM = \lim_{x \to a} f(x) \lim_{x \to a} g(x)$이 된다. $\square$
...
따라서, 위 성질은 모든 자연수에서 참이다. $\square$

  우리가 증명을 마친 위의 성질로부터 다음의 자주 쓰이는 연산 또한 유도할 수 있다.
